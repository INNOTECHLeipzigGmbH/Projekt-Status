...
- **„🔒 Angebote”-Tab** (neu, oben in den Reitern neben „Dashboard”/„Tabellenansicht”): eine eigenständige, zweite Liste – **nicht** nur eine gefilterte Ansicht der Projekte – für die Angebotsphase, ausschließlich für die Geschäftsführung sichtbar (derselbe Login wie bei „🔒 GF-Anmeldung” oben rechts; ohne Anmeldung erscheint dort nur eine Anmelde-Aufforderung, auch über die Datenbank-API nicht einsehbar):
  ...
  - „+ Angebot anlegen” öffnet direkt das Bearbeiten-Formular für ein neues Angebot; „✎ Bearbeiten” passt ein bestehendes Angebot jederzeit an (Name, Auftraggeber, PL Innotech, PL Kunde, Status, Angebotsdatum, Frist, Angebotswert &amp; Partner, Ablauf-Status, eigene ToDos)
...
- **Partner-Mehrfachauswahl mit Wert-Aufteilung** („+ Partner hinzufügen”, bei Angeboten und Projekten):
  - Statt eines einzelnen Partner-Namens lässt sich jetzt eine beliebige Liste weiterer Partner erfassen – je Partner Name, Firma, E-Mail, Telefon und der eigene **Wert-Anteil**
  - Der Gesamtwert (Angebotswert bzw. Auftragswert) wird automatisch als **Summe aus dem INNOTECH-Anteil und allen Partner-Anteilen** berechnet und angezeigt – sowohl als Gesamtsumme als auch einzeln je Partner, live während der Eingabe
  - Sobald aus einem Angebot ein Projekt geworden ist, bekommt jeder Partner zusätzlich eine Kennzeichnung **„NU-Auftrag”** (Nachunternehmer-Auftrag vorhanden ja/nein)
  - Bei **Angeboten** liegt das direkt im Angebots-Datensatz (der ganze „Angebote”-Tab ist ohnehin nur für die Geschäftsführung sichtbar: Felder `angebotswert_innotech` und `partner_liste` in der Tabelle `angebote_status`)
  - Bei **Projekten** liegt die komplette Partner-/Wertaufteilung ausschließlich im „🔒 Geschäftsführung”-Bereich (Tabelle `gf_projekt_notizen`, neue Spalten `partner_liste`/`ablauf_status`) – das bestehende, für alle sichtbare Projektfeld „Partner” (einfacher Freitext) bleibt davon unberührt und unverändert
- **Ablauf-Status „was liegt vor”** (Angebotsabfrage / Honorar / Vertrag / Bestellung / Rechnung), ebenfalls bei Angeboten und Projekten unter der Geschäftsführungs-Maske: je Punkt zwei Haken – „liegt bei AG vor” und „liegt bei uns vor” – für einen schnellen Überblick über den aktuellen Ablaufstand, ohne extra nachfragen zu müssen
