...
- **Ansprechpartner** je Projekt/Gewerk – auch für Kolleg:innen sichtbar, die nicht am Projekt arbeiten, damit sofort klar ist, wer für welches Gewerk zuständig ist:
  ...
  - Kontakt aus Outlook per Drag & Drop auf das Formular ziehen – sowohl als **vCard (.vcf)** als auch direkt als **Outlook-Kontaktelement (.msg)** (je nach Outlook-/Browser-Version zieht Outlook manchmal eine .msg- statt eine .vcf-Datei): Name, Firma, Telefon und Mail werden automatisch übernommen, Gewerk wird danach ausgewählt; alternativ komplett manuelle Eingabe. Falls Outlook beim Ziehen gar keine lesbare Datei mitgibt, erscheint eine klare Meldung mit Anleitung statt stiller Untätigkeit; die Ablagefläche lässt sich außerdem anklicken, um eine Datei ganz normal per Dateiauswahl zu laden. *Hinweis zur .msg-Unterstützung:* das Einlesen von .msg-Dateien lädt beim ersten Gebrauch eine kleine Zusatzbibliothek über eine externe Quelle (jsDelivr-CDN) nach – das erfordert eine bestehende Internetverbindung im Browser; funktioniert das Einlesen einer .msg-Datei einmal nicht, bitte kurz Rückmeldung geben, dann schaue ich mir das genauer an
...
Stand: 02.09.2026 (feste Personenauswahl bei PL Innotech/Eingangskontrolle/eigenen ToDos, Eingangskontrolle nachträglich bearbeitbar, Outlook-.msg-Unterstützung bei Ansprechpartnern)
